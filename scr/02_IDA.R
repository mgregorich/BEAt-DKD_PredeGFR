...
#                 random=list(~1|Country, ~1+Time_cat|PatID), data=data.full, control=lmeControl(opt = "optim"), method = "ML")
# ggplot(data.frame(x1=data.full$BL_uacr_log2,residual=resid(fit.init, scaled=TRUE), Time_cat=data.full$Time_cat),
#        aes(x=x1,y=residual, color=Time_cat)) +
#   geom_smooth(se=T) +
#   theme_bw() +
#   facet_wrap(~Time_cat)
# ggplot(data.frame(x1=data.full$BL_serumchol,residual=resid(fit.init, scaled=TRUE), Time_cat=data.full$Time_cat),
#        aes(x=x1,y=residual, color=Time_cat)) +
#   geom_smooth(se=T) +
#   theme_bw() +
#   facet_wrap(~Time_cat)
...
# fit.tmp1 <- lme(fixed=FU_eGFR_epi ~ (Time_cat + Time_cat * (BL_bmi + BL_age + BL_sex + BL_smoking + BL_bpsys + BL_bpdia + 
#                                                       BL_hba1c + BL_serumchol + BL_hemo + rcs(BL_uacr_log2,df=2) + BL_med_dm + BL_med_bp + BL_med_lipid)), 
#                 random=list(~1+Time_cat|PatID, ~1|Country), data=data.full, control=lmeControl(opt = "optim"), method = "ML")
# anova(fit.init, fit.tmp1)
# ggplot(data.frame(x1=data.full$BL_uacr_log2,residual=resid(fit.tmp1, scaled=TRUE), Time_cat=data.full$Time_cat),
#        aes(x=x1,y=residual, color=Time_cat)) +
#   geom_smooth(se=T) +
#   theme_bw() +
#   facet_wrap(~Time_cat)
# ggplot(data.frame(x1=data.full$BL_serumchol,residual=resid(fit.tmp1, scaled=TRUE), Time_cat=data.full$Time_cat),
#        aes(x=x1,y=residual, color=Time_cat)) +
#   geom_smooth(se=T) +
#   theme_bw() +
#   facet_wrap(~Time_cat)
...
# fit.tmp2 <- lme(fixed=FU_eGFR_epi ~ (Time_cat + Time_cat * (BL_bmi + BL_age + BL_sex + BL_smoking + BL_bpsys + BL_bpdia + 
#                                                       BL_hba1c + rcs(BL_serumchol,df=2) + BL_hemo + rcs(BL_uacr_log2,df=2) + BL_med_dm + BL_med_bp + BL_med_lipid)), 
#                 random=list(~1+Time_cat|PatID, ~1|Country), data=data.full, control=lmeControl(opt = "optim"), method = "ML")
# anova(fit.tmp1, fit.tmp2)
# ggplot(data.frame(x1=data.full$BL_serumchol,residual=resid(fit.tmp2, scaled=TRUE), Time_cat=data.full$Time_cat),
#        aes(x=x1,y=residual, color=Time_cat)) +
#   geom_smooth(se=T) +
#   theme_bw() +
#   facet_wrap(~Time_cat)
# ggplot(data.frame(x1=data.full$BL_BL_uacr_log2,residual=resid(fit.tmp2, scaled=TRUE), Time_cat=data.full$Time_cat),
#        aes(x=x1,y=residual, color=Time_cat)) +
#   geom_smooth(se=T) +
#   theme_bw() +
#   facet_wrap(~Time_cat)
...
#                  random=list(~1|Country,~1+Time_cat|PatID), data=data.full, control=lmeControl(opt = "optim"), method = "ML")
# 
# data.full.t0 <- data.full[data.full$Time_cat==0,]
# res <- LongPred_ByBase(lmeObject=fit.final, newdata = data.full.t0, Time_catVar = "Time_cat", idVar="PatID", idVar2="Country",Time_cats = unique(data.full$Time_cat), all_times=F)
# data.preds <- full_join(data.full, res$Pred[,c("PatID", "Time_cat","pred", "pred.low", "pred.upp", "slope", "slope.low", "slope.upp","prob.prog")], by=c("PatID", "Time_cat"))
# 
# plot_calibration(yobs=data.preds[data.preds$Time_cat==1,]$FU_eGFR_epi, yhat=data.preds[data.preds$Time_cat==1,]$pred, time=1)
# plot_calibration(yobs=data.preds[data.preds$Time_cat==2,]$FU_eGFR_epi, yhat=data.preds[data.preds$Time_cat==2,]$pred, time=2)
# plot_calibration(yobs=data.preds[data.preds$Time_cat==3,]$FU_eGFR_epi, yhat=data.preds[data.preds$Time_cat==3,]$pred, time=3)
# plot_calibration(yobs=data.preds[data.preds$Time_cat==4,]$FU_eGFR_epi, yhat=data.preds[data.preds$Time_cat==4,]$pred, time=4)
# plot_calibration(yobs=data.preds[data.preds$Time_cat==5,]$FU_eGFR_epi, yhat=data.preds[data.preds$Time_cat==5,]$pred, time=5)
# 
# 
# # Complex model
# fit.final <- lme(fixed=FU_eGFR_epi ~ (Time_cat + Time_cat * (BL_bmi + BL_age + BL_sex + BL_smoking + 
#                                                        (BL_bpsys + BL_bpdia)^2  + rcs(BL_serumchol, df = 2) + 
#                                                        rcs(BL_uacr_log2, df = 2) + (BL_hba1c + BL_med_dm)^2 + (BL_hemo +BL_med_bp)^2 + 
#                                                        BL_med_lipid)),
#                  random=list(~1|Country,~1+Time_cat|PatID), data=data.full, control=lmeControl(opt = "optim"), method = "ML")
# 
# data.full.t0 <- data.full[data.full$Time_cat==0,]
# res <- LongPred_ByBase(lmeObject=fit.final, newdata = data.full.t0, timeVar = "Time_cat", idVar="PatID", idVar2="Country",times = unique(data.full$Time_cat), all_times=F)
# data.preds <- full_join(data.full, res$Pred[,c("PatID", "Time_cat","pred", "pred.low", "pred.upp", "slope", "slope.low", "slope.upp","prob.prog")], by=c("PatID", "Time_cat"))
# 
# plot_calibration(yobs=data.preds[data.preds$Time_cat==1,]$FU_eGFR_epi, yhat=data.preds[data.preds$Time_cat==1,]$pred, time=1)
# plot_calibration(yobs=data.preds[data.preds$Time_cat==2,]$FU_eGFR_epi, yhat=data.preds[data.preds$Time_cat==2,]$pred, time=2)
